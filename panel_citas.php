...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Outfit', sans-serif; }
        ...
        /* CORRECCIÓN LOGOS: ESTILO UNIFICADO */
        .logo-original { 
            height: 38px; 
            width: auto; 
            object-fit: contain; 
        ...
        /* Modal Normal (Atender Cita) - Compacto */
        .modal { 
            background: white; 
            width: 95%; 
            max-width: 550px; 
        ...
        /* Modal XL (Seguimiento Global y Tarifario) - AUMENTADO DE ANCHO */
        .modal-xl-custom { 
            max-width: 1350px !important; /* Aumentado de 1000px a 1350px */
...
<body>
    ...
    <div class="modal-overlay" id="modal-preview">
        <div class="modal" style="max-width: 800px; height: 90vh;">
            <div class="modal-header">
                <h3>📸 Evidencia</h3>
                <button class="btn btn-secondary btn-icon" onclick="cerrarPreview()"><i class="ph ph-x text-lg"></i></button>
            </div>
    ...
    <script>
        ...
        // Objeto dinámico para nombres de sedes
        ...
        function getSedeFiltro() {
            ...
            // Verificamos si es visible para usar su valor
        ...
        async function cargarCatalogoSilencioso() {
            try {
                const r = await fetch('admin_catalogo_backend.php?action=list');
                DATA_CATALOGO = await r.json();
            } catch(e) { console.log("No se pudo cargar horarios del catalogo"); }
        }
        ...
        async function cargarDatosPublicos() {
            try {
                ...
                DATA_INVENTARIO = {
                    'SUPLEMENTO': d.suplementos || [],
                    'PROCEDIMIENTO': d.procedimientos || [],
                    'MEDICAMENTO': d.medicamentos || []
                };
                actualizarDatalist();
                
                // Actualizar nombres de sedes
                if(d.sedes) {
                    SEDES_NOM = {};
                    d.sedes.forEach(s => SEDES_NOM[s.id] = s.nombre);
                }
        ...
        function actualizarDatalist() {
            ...
            const dl = document.getElementById('dl-prods');
            dl.innerHTML = '';
            const items = DATA_INVENTARIO[tipo] || [];
            items.forEach(x => {
        ...
        // --- RENDERIZADO CON ACORDEÓN POR SEDE ---
        function renderTableCitas() {
            const container = document.getElementById('accordion-citas-container');
            container.innerHTML = '';
            
            // 1. Agrupar por Sede
            const agrupado = {};
            DB_CITAS.forEach(c => {
                const sId = c.sede_id || '0';
                if(!agrupado[sId]) agrupado[sId] = [];
                agrupado[sId].push(c);
            });

            // 2. Generar HTML
            if(Object.keys(agrupado).length === 0) {
                container.innerHTML = '<div style="text-align:center; padding:20px; color:#64748b;">No hay citas pendientes.</div>';
                return;
            }

            Object.keys(agrupado).forEach(sedeId => {
                const lista = agrupado[sedeId];
                const nombreSede = SEDES_NOM[sedeId] || 'Sede '+sedeId;
                
                const details = document.createElement('details');
                details.className = 'sede-accordion';
                ...
                lista.forEach((c, ix) => {
                    ...
                    rows += `
                        <tr onclick="openModal(${c.id})">
                            <td>${ix+1}</td>
                            <td>${c.fecha}<br><small style="color:var(--gray)">${c.hora}</small></td>
                            <td><span class="badge ${badgeClass}">${c.estado}</span></td>
                            <td style="font-weight:600;">${c.nom}</td>
                            <td>${c.tel||'-'}<br><b>${c.hc||'-'}</b></td>
                            <td>${formatoDoctor(c.doc)}</td>
                        </tr>
                    `;
                });

                details.innerHTML = `
                    <summary class="sede-summary">
                        <span>📍 ${nombreSede}</span>
                        <span class="count-badge">${lista.length} citas</span>
                    </summary>
                    <div class="table-container">
                        <table>
                            <thead><tr><th>#</th><th>Fecha/Hora</th><th>Estado</th><th>Paciente</th><th>HC</th><th>Doctor</th></tr></thead>
                            <tbody>${rows}</tbody>
                        </table>
                    </div>
                `;
        ...
        function renderTableGestion() {
            const container = document.getElementById('accordion-gestion-container');
            container.innerHTML = '';
            
            const agrupado = {};
            DB_GESTION.forEach(g => {
                const sId = g.sede_id || '0';
                if(!agrupado[sId]) agrupado[sId] = [];
                agrupado[sId].push(g);
            });

            if(Object.keys(agrupado).length === 0) {
                container.innerHTML = '<div style="text-align:center; padding:20px; color:#64748b;">No hay atenciones finalizadas.</div>';
                return;
            }
            ...
            Object.keys(agrupado).forEach(sedeId => {
                ...
                lista.forEach((g, ix) => {
                    ...
                    rows += `
                        <tr>
                            <td>${ix+1}</td>
                            <td>${g.fecha}<br><small>${g.hora}</small></td>
                            <td><span class="badge ${badgeClass}">${g.estado}</span></td>
                            <td style="font-weight:600;">${g.paciente}</td>
                            <td>${g.triaje||'-'}</td>
                            <td>${formatoDoctor(g.doc)}</td>
                        </tr>
                    `;
                });
                ...
                details.innerHTML = `
                    <summary class="sede-summary">
                        <span>✅ ${nombreSede}</span>
                        <span class="count-badge">${lista.length} reg.</span>
                    </summary>
                    <div class="table-container">
                        <table>
                            <thead><tr><th>#</th><th>Fecha</th><th>Estado</th><th>Paciente</th><th>Triaje</th><th>Doctor</th></tr></thead>
                            <tbody>${rows}</tbody>
                        </table>
                    </div>
                `;
        ...
        async function cargarTratamientosSede() {
            ...
            try{
                const r=await fetch(`listar_tratamientos_sede.php?sede=${sede}&inicio=${i}&fin=${n}`);
                const data=await r.json();
                RAW_TRATAMIENTOS = Array.isArray(data) ? data : [];
        ...
        function aplicarFiltrosGlobales() {
            ...
            document.getElementById('count-gestion').innerText = stats.gestion; 
        ...
        function crearFilaHTML(t) {
            ...
            // AQUI AGREGAMOS LA COLUMNA DE SEDE
            let sedeBadge = `<span class="sede-tag">${t.nombre_sede || 'Sede '+(t.sede_id||CURRENT_SEDE)}</span>`;

            tr.innerHTML = `
                <td>${sedeBadge}</td>
                ${cellFecha}
                <td>${t.nombres} ${t.apellidos} ${btnReceta}<br><a href="${linkWsp}" target="_blank" style="text-decoration:none;color:#64748b"><i class="ph ph-whatsapp-logo"></i> ${t.telefono}</a></td>
                ${cellProd}
                <td>${drOrigen}</td>
                <td style="text-align:center">${statusLabel}</td>
                <td style="text-align:center;">${btnWsp}</td>
                <td style="text-align:center; display:flex; gap:5px; justify-content:center;">${btnReal} ${btnRefuse}</td>
            `;
            return tr;
        }
        ...
        function openModal(id) { const c=DB_CITAS.find(x=>x.id==id); if(!c)return; document.getElementById('modal-id').value = id; document.getElementById('modal-paciente-info').innerText=c.nom; document.getElementById('modal-ticket').value=c.ticket; document.getElementById('modal-hc').value=c.hc; document.getElementById('modal-peso').value = ''; document.getElementById('modal-talla').value = ''; document.getElementById('modal-presion').value = ''; document.getElementById('modal-temp').value = ''; document.getElementById('modal-sat').value = ''; document.getElementById('modal-fc').value = ''; document.getElementById('modal-fr').value = ''; document.getElementById('modal-receta').value = ''; TRATAMIENTOS_TEMP = []; renderTratamientosList(); document.getElementById('trat-fecha-ini').value = c.fecha; document.getElementById('modal-detalle').classList.add('show'); }
        ...
        async function guardarAtencion() { 
            ...
            if (estadoVal !== 'CANCELADO' && fileInput.files.length === 0) { alert("⚠️ ATENCIÓN:\n\nDebe subir la FOTO DE LA RECETA obligatoriamente.\n\n(Si desea cancelar la cita, cambie el estado a 'CANCELADO')."); fileInput.focus(); fileInput.style.border = "2px solid #ef4444"; return; } else { fileInput.style.border = "1px solid #cbd5e1"; }
            const fd = new FormData(); fd.append('cita_id', idVal); fd.append('estado', estadoVal); fd.append('ticket', document.getElementById('modal-ticket').value); fd.append('hc', document.getElementById('modal-hc').value); fd.append('triaje', document.getElementById('modal-triaje').value); fd.append('peso', document.getElementById('modal-peso').value); fd.append('talla', document.getElementById('modal-talla').value); fd.append('presion', document.getElementById('modal-presion').value); fd.append('temperatura', document.getElementById('modal-temp').value); fd.append('saturacion', document.getElementById('modal-sat').value); fd.append('fc', document.getElementById('modal-fc').value); fd.append('fr', document.getElementById('modal-fr').value); fd.append('obs', document.getElementById('modal-obs').value); fd.append('re_check', document.getElementById('check-re').checked ? 1 : 0); fd.append('re_esp', document.getElementById('re-esp').value); fd.append('re_fecha', document.getElementById('re-fecha').value); fd.append('int_check', document.getElementById('check-int').checked ? 1 : 0); fd.append('int_esp', document.getElementById('int-esp').value); fd.append('trat_fecha_ini', document.getElementById('trat-fecha-ini').value); fd.append('lista_tratamientos', JSON.stringify(TRATAMIENTOS_TEMP));
            if(fileInput.files.length > 0) fd.append('receta', fileInput.files[0]);
            try { const r = await fetch('guardar_atencion.php', { method:'POST', body:fd }); const d = await r.json(); if(d.success) { closeModal(); aplicarFiltros(); showToast('success','Finalizado'); } else alert(d.message); } catch(e) { alert("Error al guardar"); } 
        }
        ...
        function verEvidencia(url) { if (!url || url === 'null') { alert("No hay archivo adjunto."); return; } url = url.trim(); const safeUrl = url.replace(/\\/g, '/'); const ext = safeUrl.split('.').pop().toLowerCase(); const content = document.getElementById('preview-content'); content.innerHTML = ''; if(ext === 'pdf') content.innerHTML = `<iframe src="${safeUrl}" width="100%" height="100%" style="border:none;"></iframe>`; else content.innerHTML = `<img src="${safeUrl}" style="max-width:100%; max-height:100%; object-fit:contain;">`; document.getElementById('modal-preview').classList.add('show'); }
