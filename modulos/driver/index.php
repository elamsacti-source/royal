...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == 'tomar') {
        // ACTUALIZACIÓN: Ahora permite tomar pedidos 'pendiente' O 'confirmado'
        $sql = "UPDATE ventas SET id_driver = ?, estado_delivery = 'en_camino' 
                WHERE id = ? AND (estado_delivery = 'pendiente' OR estado_delivery = 'confirmado')";
        
        $stmt = $pdo->prepare($sql);
        if ($stmt->execute([$id_user, $id_venta])) {
            if ($stmt->rowCount() > 0) echo json_encode(['success'=>true, 'message'=>'✅ Pedido asignado. ¡A rodar!']);
            else echo json_encode(['success'=>false, 'message'=>'❌ El pedido ya fue tomado o cancelado.']);
        } else {
...
// 3. CONSULTAS
// Mis Pedidos en Curso (Ya tomados por mí)
...
// Pedidos Disponibles (Pendientes O Confirmados por tienda)
$stmtPend = $pdo->prepare("SELECT * FROM ventas WHERE (estado_delivery = 'pendiente' OR estado_delivery = 'confirmado') ORDER BY fecha DESC");
$stmtPend->execute();
...
<head>
    ...
    <style>
        ...
        /* APP WRAPPER */
        .app-wrapper { max-width: 500px; margin: 0 auto; padding-bottom: 120px; }
        ...
        .btn-exit { color: #ef5350; font-size: 1.4rem; cursor: pointer; }

        /* LISTAS */
        .stack-container { display: flex; flex-direction: column; gap: 20px; padding: 20px; }
        .section-heading { font-size: 0.85rem; color: #666; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; margin-left: 5px; }

        /* TARJETAS */
        .card {
            background: #121212; border-radius: 16px; padding: 20px; position: relative;
            display: flex; flex-direction: column; gap: 15px; border: 1px solid #252525;
            transition: transform 0.2s;
        }
        .card:active { transform: scale(0.98); }

        .card-active { background: linear-gradient(160deg, #1b2e1e 0%, #0a0a0a 100%); border: 1px solid #2e7d32; }
        .card-active::before { content: ''; position: absolute; left: 0; top: 15px; bottom: 15px; width: 4px; background: #4caf50; border-radius: 0 4px 4px 0; }
        .card-pending { border-left: 4px solid #FFD700; }

        /* BADGES */
        .card-header { display: flex; justify-content: space-between; align-items: center; }
        .badge { padding: 5px 12px; border-radius: 20px; font-size: 0.7rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; margin-right: 5px; }
        
        .badge-ruta { background: rgba(76, 175, 80, 0.2); color: #4caf50; border: 1px solid rgba(76, 175, 80, 0.3); }
        
        /* Badges dinámicos para estado de tienda */
        .badge-confirmado { background: rgba(33, 150, 243, 0.2); color: #2196f3; border: 1px solid #2196f3; }
        .badge-pendiente { background: #333; color: #aaa; border: 1px solid #555; }
        ...
        .meta-info { display: flex; gap: 15px; font-size: 0.85rem; color: #888; border-top: 1px solid #222; padding-top: 12px; }
        .meta-item { display: flex; align-items: center; gap: 6px; }

        /* BOTONES */
        .action-buttons { display: flex; flex-direction: column; gap: 10px; margin-top: 5px; }
        .btn { width: 100%; padding: 14px; border-radius: 12px; border: none; font-size: 0.95rem; font-weight: 700; cursor: pointer; text-decoration: none; display: flex; align-items: center; justify-content: center; gap: 10px; }
        ...
        .fab-refresh { position: fixed; bottom: 30px; right: 25px; width: 56px; height: 56px; border-radius: 50%; background: #111; border: 2px solid #FFD700; color: #FFD700; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; box-shadow: 0 5px 20px rgba(0,0,0,0.6); z-index: 200; cursor: pointer; }
    ...
    <div class="stack-container">
        ...
        <?php if (count($pendientes) > 0): ?>
            <?php foreach($pendientes as $p): ?>
                <div class="card card-pending fade-in">
                    <div class="card-header">
                        <div>
                            <?php if($p['estado_delivery'] == 'confirmado'): ?>
                                <span class="badge badge-confirmado"><i class="fa-solid fa-check"></i> LISTO</span>
                            <?php else: ?>
                                <span class="badge badge-pendiente">NUEVO</span>
                            <?php endif; ?>
                            
                            <span style="font-size:0.75rem; color:#888; border:1px solid #444; padding:3px 8px; border-radius:10px;">
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p>Todo tranquilo por ahora.<br>No hay pedidos pendientes ni confirmados.</p>
            </div>
        <?php endif; ?>
