...
<style>
    ...
    .p-huaura { background: #eff6ff; border-color: #3b82f6; border-width: 2px; }
    .p-huacho { background: #f0fdf4; border-color: #bbf7d0; }
    .p-mmundo { background: #fffbeb; border-color: #fde68a; }
...
<script>
    ...
    $("#src_linea").autocomplete({ source: (req, res) => $.post('admin_homologar.php', {action:'search_linea', term:req.term}, res, 'json'), select: (e, ui) => $("#m_linea_cod").val(ui.item.id) });
    $("#src_marca").autocomplete({ source: (req, res) => $.post('admin_homologar.php', {action:'search_marca', term:req.term}, res, 'json'), select: (e, ui) => $("#m_marca_cod").val(ui.item.id) });
    ...
    function syncAll() {
        ...
        $.post('admin_homologar.php', d, (res) => {
            if(res.success) {
                $("#alertBox").text("✅ "+res.msg).fadeIn().delay(3000).fadeOut(); $("#src_huaura").val('').focus(); $("#master_panel").hide();
                resetCard('huacho'); resetCard('mm'); $("#btn_sync").prop('disabled',false).html('<i class="ph ph-arrows-left-right"></i> SINCRONIZAR TODO');
