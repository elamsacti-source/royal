...
// IMPORTADOR MAESTRO: Lee datos a MySQL y guarda copia física del DBF original para exportación futura.
...
function readCSV($file) {
    ...
    if (($handle = fopen($file, "r")) !== FALSE) {
        ...
        while (($data = fgetcsv($handle, 0, $delimiter)) !== FALSE) {
            ...
            if (!$header) { $header = array_map(function($h) { return strtoupper(trim($h)); }, $data); } 
            else { if (count($header) === count($data)) $rows[] = array_combine($header, $data); }
        }
        fclose($handle);
    }
    return $rows;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['dbf_file'])) {
    try {
        ...
        // ---------------------------------------------------------
        // GUARDAR BACKUP MAESTRO (SOLO SI ES DBF)
        // ---------------------------------------------------------
        if ($ext !== 'csv' && $ext !== 'txt') {
            $backupDir = __DIR__ . '/dbf_backups/';
            if (!file_exists($backupDir)) { mkdir($backupDir, 0777, true); }
            
            // Prefijo según el tipo de archivo para no mezclarlos
            $prefix = ($tipo === 'principios') ? 'maestro_principios_' : 'maestro_inv_';
            $backupPath = $backupDir . $prefix . $sede . ".dbf";
            
            if (!move_uploaded_file($tmpFile, $backupPath)) {
                // Fallback si move falla
                copy($tmpFile, $backupPath);
            }
            // Apuntamos tmpFile al nuevo backup para seguir leyendo
            $tmpFile = $backupPath; 
        }
        // ---------------------------------------------------------
        ...
        if ($tipo === 'principios') {
            ...
            $stmt->close();
            echo json_encode(['success'=>true, 'message'=>"ZETH19: $total principios cargados y backup actualizado."]);
        } 
        else {
            // Carga Zeth70 (Inventario)
            ...
            foreach ($dataRows as $row) {
                ...
                if (empty($costoRaw) || floatval($costoRaw) == 0) $costoRaw = findVal($row, ['PRECOS', 'PRE_COS']);
            ...
            echo json_encode(['success'=>true, 'message'=>"Procesado. Backup Zeth70 actualizado. Act: $act, Nuevos: $nue."]);
        }
